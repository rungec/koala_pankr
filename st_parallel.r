#Run sf and raster functions in parallel

#uses snow & raster
rast_parallel <- function(x, y, n_cores, ...){
  if (n_cores == 1){
    result <- raster::extract(x, y, ...)  
  } else {
...
st_parallel <- function(sf_df, sf_func, n_cores, ...){
  ...
  if (n_cores == 1){
    ...
    split_results <- split(sf_df, split_vector) %>%
      parallelsugar::mclapply(function(x) sf_func(x, ...), mc.cores = n_cores)
